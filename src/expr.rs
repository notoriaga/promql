...
fn function_args<'a, I, C>(
    recursion_level: usize,
    opts: &'a ParserOptions,
) -> impl FnMut(I) -> IResult<I, Vec<Node>> + 'a
where
    I: Clone
        + Copy
        + AsBytes
...
fn with_modifier<'a, I, C>(
    opts: &'a ParserOptions,
    literal: &'static str,
    op: fn(Option<OpMod>) -> Op,
) -> impl FnMut(I) -> IResult<I, Op> + 'a
where
    I: Clone
        + AsBytes
        + Compare<&'static str>
...
fn with_bool_modifier<'a, I, C, O: Fn(bool, Option<OpMod>) -> Op + 'a>(
    opts: &'a ParserOptions,
    literal: &'static str,
    op: O,
) -> impl FnMut(I) -> IResult<I, Op> + 'a
                    ...
                    opt(map(
                        tuple_separated!(
                            ...
                            (
                                ...
                                map(opt(|i| label_list(i, opts)), |labels| labels
                                    .unwrap_or_default()),
                            )
                        ),
    ...
    <I as InputIter>::IterElem: Clone,
{
    ...
    */
    ...
    collapse_ops(
        ...
        &mut ops,
        false,
        op_matcher!(Op::Mul, Op::Div, Op::Mod),
    );
    ...
    collapse_ops(
        ...
        &mut ops,
        false,
        op_matcher!(Op::Eq, Op::Ne, Op::Le, Op::Ge, Op::Lt, Op::Gt),
    );
...
mod tests {
    ...
    fn scalar_single(input: &str, output: f32) {
        assert_eq!(
            expression(0, input, &Default::default()),
            Ok(("", Scalar(output)))
        );
    }
    ...
    fn ops() {
        assert_eq!(
            expression(0, "foo > bar != 0 and 15.5 < xyzzy", &Default::default(),),
            Ok((
                "",
                operator(
                    operator(
        ...
        assert_eq!(
            expression(0, "foo + bar - baz <= quux + xyzzy", &Default::default(),),
            Ok((
                "",
                operator(
                    operator(
        ...
        assert_eq!(
            expression(0, "foo + bar % baz", &Default::default(),),
            Ok((
                "",
                operator(
                    vector("foo"),
        ...
        assert_eq!(
            expression(0, "x^y^z", &Default::default(),),
            Ok((
                "",
                operator(
                    vector("x"),
        ...
        assert_eq!(
            expression(0, "(a+b)*c", &Default::default(),),
            Ok((
                "",
                operator(
    ...
    fn unary() {
        assert_eq!(
            expression(0, "a + -b", &Default::default(),),
            Ok((
                "",
        ...
        assert_eq!(
            expression(0, "a ^ - 1 - b", &Default::default(),),
            Ok((
                "",
                operator(
    ...
    fn comments() {
        ...
        assert_eq!(
            ...
            Ok(("", operator(vector("foo"), Div(None), vector("baz"))))
        );
