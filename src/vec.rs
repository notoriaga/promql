    ...
    &'static str: FindToken<C>,
{
    delimited(
        ...
        surrounded_ws(separated_list0(
            surrounded_ws(char(',')),
            map(
                tuple_separated!(ws_or_comment(opts), (label_name, label_op, string,)),
    ...
    &'static str: FindToken<C>,
{
    map_res(
        recognize(tuple((
            ...
            many0(alt((
                alphanumeric1,
                is_a(if opts.allow_dots { "_:." } else { "_:" }),
            ))),
        ))),
...
mod tests {
    ...
    fn instant_vectors_period() {
        ...
        for q in ["foo.bar{}", "foo.bar { }"] {
            assert_eq!(
                vector(q, &ParserOptions::new().allow_dots(true).build(),),
                Ok((
                    "",
                    Vector {
    ...
    fn instant_vectors_no_period() {
        ...
        for (q, tail) in [("foo.bar{}", ".bar{}"), ("foo.bar { }", ".bar { }")] {
            assert_eq!(
                vector(q, &ParserOptions::new().allow_dots(false).build(),),
                Ok((
                    tail,
                    Vector {
                        labels: vec![LabelMatch {
    ...
    fn instant_vectors(allow_dots: bool) {
        ...
        for q in ["{lorem=~\"ipsum\"}", "{ lorem =~ \"ipsum\" }"] {
            assert_eq!(
                vector(q, &opts),
                Ok((
                    "",
    ...
    fn modified_vectors(opts: &ParserOptions) {
        ...
        for q in ["foo{bar!~'baz'}", "foo { bar !~ 'baz' }"] {
            for f in [
                |instant, labels, opts| modified_vectors_for_instant(instant, labels, opts),
                |instant, labels, opts| {
                    modified_vectors_for_instant_concat_offset(instant, labels, opts, true)
                },
            ] {
                f(
                    q,
        ...
        for q in ["{instance!=`localhost`}", "{ instance != `localhost` }"] {
            for f in [
                |instant, labels, opts| modified_vectors_for_instant(instant, labels, opts),
                |instant, labels, opts| {
                    modified_vectors_for_instant_concat_offset(instant, labels, opts, true)
    ...
    fn modified_vectors_for_instant(
        ...
        for q in [format!("{}[1m]", instant), format!("{} [ 1m ]", instant)] {
            assert_eq!(vector(q.as_str(), opts), v("", Some(60.), None),);
        }

        // TODO failing "offset5m"
        let q = format!("{} offset 5m", instant);
        assert_eq!(vector(q.as_str(), opts), v("", None, Some(300.)),);
        ...
        for q in [
            ...
            assert_eq!(vector(q.as_str(), opts), v("", Some(60.), Some(300.)),);
        }
        // for "{}offset" without space see `modified_vectors_for_instant_concat_offset`
        ...
        assert_eq!(vector(q.as_str(), opts), v("[1m]", None, Some(300.)),);
    ...
    fn ranges() {
        ...
        assert_eq!(range_literal("500ms", &opts), Ok(("", 0.5)));

        let opts = ParserOptions::new().ms_duration(false).build();
        assert_eq!(range_literal("500ms", &opts), Ok(("s", 500. * 60.)));
    }
}
