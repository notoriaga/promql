...
pub(crate) fn ws_or_comment<'a, I, C>(
    opts: &'a ParserOptions,
) -> impl FnMut(I) -> IResult<I, ()> + 'a
where
    I: Clone
        + Compare<&'static str>
        + InputIter<Item = C>
...
pub(crate) fn surrounded_ws_or_comment<'a, I, C, O, P>(
    opts: &'a ParserOptions,
    parser: P,
) -> impl FnMut(I) -> IResult<I, O> + 'a
where
...
mod tests {
    ...
    fn comments() {
        ...
        assert_eq!(ws_or_comment(&opts)(src), Ok(("", ())),);

        let opts = ParserOptions::new().comments(false).build();

        assert_eq!(ws_or_comment(&opts)(src), Ok(("# whatever\n", ())),);
    }
}
